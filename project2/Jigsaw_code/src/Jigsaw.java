...
public class Jigsaw {
	...
	while (!openList.isEmpty()) {
	    ...
	    searchedNodesNum++;
	    Vector<JigsawNode> adjacentJNodes = findFollowJNodes(currentJNode);
		    
	    for (JigsawNode node : adjacentJNodes) {
	...
	// (2) 如果openList为空，或者访问节点数大于maxNodesNum个，
	// 则搜索失败，问题无解;否则循环直到求解成功
	while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
	    ...
	    // (2-3)寻找所有与currentJNode邻接且未曾被访问的节点,
	    // 将它们按代价估值从小到大排序插入openList中
	...
	int s1 = estimateValue1(jNode);
	int s2 = estimateValue2(jNode);
	int s3 = estimateValue3(jNode);
	    
	jNode.setEstimatedValue(s1 + s2 * 6 + s3 * 4);
    }

    /**
     * the number of the wrong next code
     */
    private int  estimateValue1(JigsawNode jNode) {
     ...
     * Tax distance
     */
    private int estimateValue2(JigsawNode jNode) {
	...
	for(int index =1 ; index <= dimension * dimension; index++){
		...
		realRow = (jNode.getNodesState()[index] - 1) / dimension;
		realCol = (jNode.getNodesState()[index] - 1) % dimension;
		curRow = (index - 1) / dimension;
		curCol = (index - 1) % dimension;
	
		s += Math.abs(realRow - curRow) + Math.abs(realCol - curCol);
	    }
	}
	return s;
    }
    /**
     * the number of the wrong position code
     */
    private int estimateValue3(JigsawNode jNode) {
